...
| log.roll.ms | The maximum time before a new log segment is rolled out \(in milliseconds\). If not set, the value in log.roll.hours is used 日志片段文件被轮训覆盖的最大时间| long | null |  | high |
| log.segment.bytes | The maximum size of a single log file 单个日志文件的最大值| int | 1073741824 | \[14,...\] | high |
